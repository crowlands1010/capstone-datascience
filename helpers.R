...
sentToken <- Maxent_Sent_Token_Annotator()
wordToken <- Maxent_Word_Token_Annotator()
...
get_default <- function(text) {
    if (length(text) > 0) {
        a2 <- annotate(as.String(text), list(sentToken, wordToken))
        a3 <- annotate(as.String(text), posTag, a2)
...
get_pred <- function(text) {
    if (text != " ") { 
        ...
        len3 <- length(trigram[trigram[n2 == w2 & n1 == w1]]$freq)
        len2 <- length(bigram[bigram[n1 == w1]]$freq)
        ...
        if (len3 > 0) {
            for (i in 1:len3) {
                matches[i, 1] <- trigram[trigram[n2 == w2 & n1 == w1]]$n0[i]
                cnt2 <- length(bigram[bigram[n1 == w1 & n0 == matches[i, 1]]]$freq)
                cnt1 <- length(unigram[unigram[n0 == matches[i, 1]]]$freq)
                if (cnt2 > 0) freq2 <- bigram[bigram[n1 == w1 & 
                    n0 == matches[i, 1]]]$freq else freq2 <- 0
                if (cnt1 > 0) freq1 <- unigram[unigram[n0 == matches[i, 1]]]$freq else freq1 <- 0
                matches[i, 2] <- trigram[trigram[n2 == w2 & n1 == w1]]$freq[i] * 
                    l1 + freq2 * l2 + freq1 * l3     
            }
        }
        if (len2 > 0) {
            for (i in sum(len3, 1):sum(len3, len2)) {
                matches[i, 1] <- bigram[bigram[n1 == w1]]$n0[i - len3]
                cnt1 <- length(unigram[unigram[n0 == matches[i, 1]]]$freq)
                if (cnt1 > 0) freq1 <- unigram[unigram[n0 == matches[i, 1]]]$freq else freq1 <- 0
                matches[i, 2] <- bigram[bigram[n1 == w1]]$freq[i - len3] * l2 + freq1 * l3   
            }
        }
